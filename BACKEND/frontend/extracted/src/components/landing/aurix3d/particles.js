...
class Particle {
    ...
    update(frameScale = 1) {
        const spd = (.3 + this.depth * .7) * frameScale;
        this.x += this.vx * spd;
        this.y += this.vy * spd;
        this.life += (.0022 + this.depth * .001) * frameScale;
        this.twinkle += this.tSpeed * frameScale;
...
export class ParticleSystem {
    ...
    init() {
        ...
        const pCount = window.innerWidth < 768 ? 90 : 150;
        const sCount = window.innerWidth < 768 ? 10 : 18;
    ...
    update(dt = 1 / 60) {
        ...
        this.streams.forEach(s => s.update(frameScale));
        this.particles.forEach(p => p.update(frameScale));

        this.sortTimer += dt;
        if (this.sortTimer >= 0.2) {
            this.particles.sort((a, b) => a.depth - b.depth);
            this.sortTimer = 0;
        }
    }
